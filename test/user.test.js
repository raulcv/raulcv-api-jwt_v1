
/* eslint handle-callback-err: "off"*/
require('dotenv-safe').config()

process.env.NODE_ENV = 'test'

const User = require('../src/models/user')
const { faker } = require('@faker-js/faker');
const chai = require('chai')
const chaiHttp = require('chai-http')
const server = require('../src/server')
// eslint-disable-next-line no-unused-vars
const should = chai.should()
const loginDetails = {
    admin: {
        // role: '620099e0284dd6469b43131c',
        role: 'admin',
        email: 'admin@admin.com',
        password: 'Admin 2022'
    },
    user: {
        role: 'public',
        email: 'user@user.com',
        password: 'User 2022'
    }
}
const tokens = {
    admin: '',
    user: ''
}
const role = {
    admin: '620099e0284dd6469b43131c',
    public: '6200acdb658b3a7939924b8e'
}
const email = faker.internet.email()
const createdID = []

chai.use(chaiHttp)

describe('*********** USERS ***********', () => {
    describe('/POST login', () => {
        it('it should GET token as admin', (done) => {
            chai
                .request(server)
                .post('/login')
                .send(loginDetails.admin)
                .end((err, res) => {
                    res.should.have.status(200)
                    res.body.should.be.an('object')
                    res.body.should.have.property('token')
                    tokens.admin = res.body.token
                    done()
                })
        })
        it('it should GET token as public user', (done) => {
            chai
                .request(server)
                .post('/login')
                .send(loginDetails.user)
                .end((err, res) => {
                    res.should.have.status(200)
                    res.body.should.be.an('object')
                    res.body.should.have.property('token')
                    tokens.user = res.body.token
                    done()
                })
        })
    })
    describe('/GET users', () => {
        it('it should NOT be able to consume the route since no token was sent', (done) => {
            chai
                .request(server)
                .get('/api/user')
                .end((err, res) => {
                    res.should.have.status(401)
                    done()
                })
        })
        it('it should GET all the users', (done) => {
            chai
                .request(server)
                .get('/api/user')
                .set('Authorization', `Bearer ${tokens.admin}`)
                .end((err, res) => {
                    res.should.have.status(200)
                    res.body.should.be.an('object')
                    res.body.docs.should.be.a('array')
                    done()
                })
        })
        it('it should GET the users with filters', (done) => {
            chai
                .request(server)
                .get('/api/user?filter=admin&fields=name,email,city,country,phone')
                .set('Authorization', `Bearer ${tokens.admin}`)
                .end((err, res) => {
                    res.should.have.status(200)
                    res.body.should.be.an('object')
                    res.body.docs.should.be.a('array')
                    res.body.docs.should.have.lengthOf(1)
                    res.body.docs[0].should.have.property('email').eql('admin@admin.com')
                    done()
                })
        })
    })

    describe('/POST user', () => {
        it('it should NOT POST a user without name', (done) => {
            const user = {}
            chai
                .request(server)
                .post('/api/user')
                .set('Authorization', `Bearer ${tokens.admin}`)
                .send(user)
                .end((err, res) => {
                    res.should.have.status(422)
                    res.body.should.be.a('object')
                    res.body.should.have.property('errors')
                    done()
                })
        })
        it('it should POST a user ', (done) => {
            const user = {
                name: faker.name.findName(),
                email,
                password: faker.internet.password(),
                role: role.admin,
                urlTwitter: faker.internet.url(),
                urlGitHub: faker.internet.url(),
                phone: faker.phone.phoneNumber(),
                city: faker.address.city(),
                country: faker.address.country()
            }
            // console.log(user)
            chai
                .request(server)
                .post('/api/user')
                .set('Authorization', `Bearer ${tokens.admin}`)
                .send(user)
                .end((err, res) => {
                    // console.log(res.body)
                    res.should.have.status(201)
                    res.body.should.be.a('object')
                    res.body.should.include.keys('_id', 'name', 'email', 'verification')
                    createdID.push(res.body._id)
                    done()
                })
        })
        it('it should NOT POST a user with email that already exists', (done) => {
            const user = {
                name: faker.name.findName(),
                email,
                password: faker.internet.password(),
                role: role.admin
            }
            chai
                .request(server)
                .post('/api/user')
                .set('Authorization', `Bearer ${tokens.admin}`)
                .send(user)
                .end((err, res) => {
                    res.should.have.status(422)
                    res.body.should.be.a('object')
                    res.body.should.have.property('errors')
                    done()
                })
        })
        it('it should NOT POST a user with not known role', (done) => {
            const user = {
                name: faker.name.findName(),
                email,
                password: faker.internet.password(),
                role: faker.random.word()
            }
            chai
                .request(server)
                .post('/api/user')
                .set('Authorization', `Bearer ${tokens.admin}`)
                .send(user)
                .end((err, res) => {
                    res.should.have.status(422)
                    res.body.should.be.a('object')
                    res.body.should.have.property('errors')
                    done()
                })
        })
    })
    
    describe('/GET/:id user', () => {
        it('it should GET a user by the given id', (done) => {
            const id = createdID.slice(-1).pop()
            chai
                .request(server)
                .get(`/api/user/${id}`)
                .set('Authorization', `Bearer ${tokens.admin}`)
                .end((error, res) => {
                    // console.log(res.body)
                    res.should.have.status(200)
                    res.body.should.be.a('object')
                    res.body.should.have.property('name')
                    res.body.should.have.property('_id').eql(id)
                    done()
                })
        })
    })
    
    describe('/PATCH/:id user', () => {
        it('it should UPDATE a user given the id', (done) => {
            const id = createdID.slice(-1).pop()
            const user = {
                name: 'JS123456',
                email: 'emailthatalreadyexists@email.com',
                role: role.admin,
                urlTwitter: faker.internet.url(),
                urlGitHub: faker.internet.url(),
                phone: faker.phone.phoneNumber(),
                city: faker.address.city(),
                country: faker.address.country()
            }
            chai
                .request(server)
                .patch(`/api/user/${id}`)
                .set('Authorization', `Bearer ${tokens.admin}`)
                .send(user)
                .end((error, res) => {
                    res.should.have.status(200)
                    res.body.should.be.a('object')
                    res.body.should.have.property('_id').eql(id)
                    res.body.should.have.property('name').eql('JS123456')
                    res.body.should.have
                        .property('email')
                        .eql('emailthatalreadyexists@email.com')
                    createdID.push(res.body._id)
                    done()
                })
        })
        it('it should NOT UPDATE a user with email that already exists', (done) => {
            const id = createdID.slice(-1).pop()
            const user = {
                name: faker.name.findName(),
                email: 'admin@admin.com',
                role: 'admin'
            }
            chai
                .request(server)
                .patch(`/api/user/${id}`)
                .set('Authorization', `Bearer ${tokens.admin}`)
                .send(user)
                .end((err, res) => {
                    res.should.have.status(422)
                    res.body.should.be.a('object')
                    res.body.should.have.property('errors')
                    done()
                })
        })
        it('it should NOT UPDATE another user if is not an admin', (done) => {
            const id = createdID.slice(-1).pop()
            const user = {
                name: faker.name.findName(),
                email: 'toto@toto.com',
                role: role.public
            }
            chai
                .request(server)
                .patch(`/api/user/${id}`)
                .set('Authorization', `Bearer ${tokens.user}`)
                .send(user)
                .end((err, res) => {
                    console.log(res.body)
                    res.should.have.status(401)
                    res.body.should.be.a('object')
                    res.body.should.have.property('errors')
                    done()
                })
        })
    })
    describe('/PATCH/: ADD & REMOVE ROLE id user', () => {
        it('it should ADD new Role of user given the id', (done) => {
            const id = createdID.slice(-1).pop()
            const user = { role: "6200acdb658b3a7939924b8e" //PUBLIC ROLE ID
        }
            chai
                .request(server)
                .patch(`/api/user/addRole/${id}`)
                .set('Authorization', `Bearer ${tokens.admin}`)
                .send(user)
                .end((error, res) => {
                    console.log(res.body)
                    res.should.have.status(200)
                    res.body.should.be.a('object')
                    res.body.should.have.property('_id').eql(id)
                    createdID.push(res.body._id)
                    done()
                })
        })
        it('it should NOT ADD a role of user that already exists', (done) => {
            const id = createdID.slice(-1).pop()
            const user = { 
                role: '620099e0284dd6469b43131c' //ADMIN ROLE ID
            }
            chai
                .request(server)
                .patch(`/api/user/addRole/${id}`)
                .set('Authorization', `Bearer ${tokens.admin}`)
                .send(user)
                .end((err, res) => {
                    res.should.have.status(422)
                    res.body.should.be.a('object')
                    res.body.should.have.property('errors')
                    done()
                })
        })
        it('it should Remove a Role of user given the id', (done) => {
            const id = createdID.slice(-1).pop()
            const user = { role: "620099e0284dd6469b43131c" }
            chai
                .request(server)
                .patch(`/api/user/removeRole/${id}`)
                .set('Authorization', `Bearer ${tokens.admin}`)
                .send(user)
                .end((error, res) => {
                    res.should.have.status(200)
                    res.body.should.be.a('object')
                    res.body.should.have.property('_id').eql(id)
                    createdID.push(res.body._id)
                    done()
                })
        })
    })

    describe('/DELETE/:id user', () => {
        it('it should DELETE a user given the id', (done) => {
            const user = {
                name: faker.name.findName(),
                email: faker.internet.email(),
                password: faker.internet.password(),
                role: role.admin,
                urlTwitter: faker.internet.url(),
                urlGitHub: faker.internet.url(),
                phone: faker.phone.phoneNumber(),
                city: faker.address.city(),
                country: faker.address.country()
            }
            chai
                .request(server)
                .post('/api/user')
                .set('Authorization', `Bearer ${tokens.admin}`)
                .send(user)
                .end((err, res) => {
                    res.should.have.status(201)
                    res.body.should.be.a('object')
                    res.body.should.include.keys('_id', 'name', 'email', 'verification')
                    chai
                        .request(server)
                        .delete(`/api/user/${res.body._id}`)
                        .set('Authorization', `Bearer ${tokens.admin}`)
                        .end((error, result) => {
                            result.should.have.status(200)
                            result.body.should.be.a('object')
                            result.body.should.have.property('msg').eql('It was deleted')
                            done()
                        })
                })
        })
    })

    after(() => {
        createdID.forEach((id) => {
            User.findByIdAndRemove(id, (err) => {
                if (err) {
                    console.log(err)
                }
            })
        })
    })

})